...
//		halfvz = q0q0 - 0.5f + q3q3;
        ...
        //20170912 by tht. test
        halfvx = 2.0f * (q1q3 - q0q2);
		halfvy = 2.0f * (q0q1 + q2q3);
		halfvz = 1.0f - 2 * (q1q1 + q2q2);
...
#define so3_comp_params_Kp 40.0f
#define so3_comp_params_Ki  0.05f
#define GYRO_SCALE_MIN  0.002f
...
//        euler[0] = atan2f(-Rot_matrix[6], Rot_matrix[8]);	//! Roll
//		euler[1] = asinf(Rot_matrix[7]);									//! Pitch
//		euler[2] = atan2f(Rot_matrix[1], Rot_matrix[4]);	
//		
		//DCM . ground to body
		for(i=0;i<9;i++)
		{
        ...
        //改为精度为0.01度的16位有符号整数，用于测试
		imu.roll=(int16_t)(euler[0] * 180.0f * 100.0f / M_PI_F);
		imu.pitch=(int16_t)(euler[1] * 180.0f * 100.0f / M_PI_F);
		imu.yaw=(int16_t)(euler[2] * 180.0f * 100.0f / M_PI_F);
} 
